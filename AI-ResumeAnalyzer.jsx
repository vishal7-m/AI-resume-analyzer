...
export default function App() {
  ...
  useEffect(() => {
    if (analysis && activeTab === "overview") {
  ...
  function renderCharts() {
    ...
    const { atsScore, skills, matchedSkills, missingSkills, jdSkills } = analysis;

    if (chartInstances.current.ats) { chartInstances.current.ats.destroy(); }
    const atsCanvas = document.getElementById("ats-donut");
    if (atsCanvas) {
      chartInstances.current.ats = new Chart(atsCanvas, {
        type: "doughnut",
        data: { labels: ["ATS Score", "Gap"], datasets: [{ data: [atsScore.total, 100 - atsScore.total], backgroundColor: ["#1D9E75", "#E1F5EE"], borderWidth: 0 }] },
        options: { responsive: true, maintainAspectRatio: false, cutout: "75%", plugins: { legend: { display: false }, tooltip: { callbacks: { label: (c) => c.label + ": " + c.raw + "%" } } } }
      });
    }

    if (chartInstances.current.match) { chartInstances.current.match.destroy(); }
    const matchCanvas = document.getElementById("match-donut");
    if (matchCanvas) {
      const matchPct = jdSkills.length > 0 ? Math.round((matchedSkills.length / jdSkills.length) * 100) : 0;
      chartInstances.current.match = new Chart(matchCanvas, {
        type: "doughnut",
        data: { labels: ["Matched", "Missing"], datasets: [{ data: [matchPct, 100 - matchPct], backgroundColor: ["#378ADD", "#E6F1FB"], borderWidth: 0 }] },
        options: { responsive: true, maintainAspectRatio: false, cutout: "75%", plugins: { legend: { display: false } } }
      });
    }

    if (chartInstances.current.skills) { chartInstances.current.skills.destroy(); }
    const skillsCanvas = document.getElementById("skills-bar");
    if (skillsCanvas) {
      const breakdown = atsScore.breakdown;
      chartInstances.current.skills = new Chart(skillsCanvas, {
        type: "bar",
        data: { labels: ["Sections", "Keywords", "Length", "Skills", "Structure"], datasets: [{ label: "Score", data: [breakdown.sections, breakdown.keywords, breakdown.length, breakdown.skillsCoverage, breakdown.structure], backgroundColor: ["#534AB7","#1D9E75","#378ADD","#D85A30","#D4537E"], borderRadius: 6, borderWidth: 0 }] },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { max: 35, ticks: { color: "#888" }, grid: { color: "rgba(0,0,0,0.05)" } }, x: { ticks: { color: "#888" }, grid: { display: false } } } }
      });
    }
  }
        ...
        <div style={{ width: "100%", maxWidth: 420 }}>
          <div style={{ textAlign: "center", marginBottom: "2rem" }}>
            <div style={{ width: 52, height: 52, borderRadius: "14px", background: "#534AB7", display: "inline-flex", alignItems: "center", justifyContent: "center", marginBottom: "1rem" }}>
              <i className="ti ti-file-analytics" style={{ fontSize: 26, color: "#fff" }} aria-hidden="true"></i>
            </div>
        ...
        <header style={{ background: "var(--color-background-primary)", borderBottom: "0.5px solid var(--color-border-tertiary)", padding: "0 2rem", display: "flex", alignItems: "center", justifyContent: "space-between", height: 56 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 32, height: 32, background: "#534AB7", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center" }}>
              <i className="ti ti-file-analytics" style={{ color: "#fff", fontSize: 16 }} aria-hidden="true"></i>
            </div>
        ...
        <div style={{ maxWidth: 860, margin: "0 auto", padding: "2rem 1rem" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.25rem", marginBottom: "1.25rem" }}>
            <div style={{ background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-lg)", padding: "1.25rem" }}>
              <label style={{ fontSize: 14, fontWeight: 500, display: "block", marginBottom: "0.75rem" }}><i className="ti ti-upload" aria-hidden="true" style={{ marginRight: 6 }}></i>Resume</label>
              <div onClick={() => fileRef.current.click()} style={{ border: "1.5px dashed var(--color-border-secondary)", borderRadius: "var(--border-radius-md)", padding: "1.5rem", textAlign: "center", cursor: "pointer", marginBottom: "0.75rem", transition: "border-color 0.15s" }} onMouseOver={e => e.currentTarget.style.borderColor = "#534AB7"} onMouseOut={e => e.currentTarget.style.borderColor = "var(--color-border-secondary)"}>
                <i className="ti ti-file-upload" style={{ fontSize: 28, color: "#534AB7", display: "block", marginBottom: 6 }} aria-hidden="true"></i>
            ...
            <div style={{ background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-lg)", padding: "1.25rem" }}>
              <label style={{ fontSize: 14, fontWeight: 500, display: "block", marginBottom: "0.75rem" }}><i className="ti ti-briefcase" aria-hidden="true" style={{ marginRight: 6 }}></i>Job description <span style={{ color: "var(--color-text-secondary)", fontWeight: 400, fontSize: 12 }}>(optional)</span></label>
          ...
          <div style={{ display: "flex", gap: 12, alignItems: "center", justifyContent: "space-between" }}>
            <button onClick={handleAnalyze} disabled={loading || !resumeText.trim()} style={{ padding: "11px 28px", background: "#534AB7", color: "#fff", border: "none", borderRadius: "var(--border-radius-md)", fontSize: 14, fontWeight: 500, cursor: loading || !resumeText.trim() ? "not-allowed" : "pointer", opacity: !resumeText.trim() ? 0.5 : 1, display: "flex", alignItems: "center", gap: 8 }}>
              {loading ? <><i className="ti ti-loader" style={{ fontSize: 16, animation: "spin 1s linear infinite" }} aria-hidden="true"></i>{loadingMsg}</> : <><i className="ti ti-brain" style={{ fontSize: 16 }} aria-hidden="true"></i>Analyze resume</>}
            </button>
            {history.length > 0 && <button onClick={() => setPage("dashboard")} style={{ padding: "11px 20px", background: "none", border: "0.5px solid var(--color-border-secondary)", borderRadius: "var(--border-radius-md)", fontSize: 14, cursor: "pointer", color: "var(--color-text-secondary)" }}>
              <i className="ti ti-history" style={{ fontSize: 16, marginRight: 6 }} aria-hidden="true"></i>View last analysis
            </button>}
          </div>
      ...
      <div style={{ minHeight: "100vh", background: "var(--color-background-tertiary)" }}>
        ...
        <div style={{ maxWidth: 960, margin: "0 auto", padding: "1.5rem 1rem" }}>
          <div style={{ display: "flex", gap: 12, marginBottom: "1.25rem", flexWrap: "wrap" }}>
            {tabs.map(t => (
              <button key={t.id} onClick={() => setActiveTab(t.id)} style={{ padding: "7px 14px", borderRadius: "var(--border-radius-md)", border: activeTab === t.id ? "none" : "0.5px solid var(--color-border-secondary)", background: activeTab === t.id ? "#534AB7" : "var(--color-background-primary)", color: activeTab === t.id ? "#fff" : "var(--color-text-secondary)", fontSize: 13, cursor: "pointer", fontWeight: activeTab === t.id ? 500 : 400, display: "flex", alignItems: "center", gap: 6 }}>
                <i className={`ti ${t.icon}`} style={{ fontSize: 14 }} aria-hidden="true"></i>{t.label}
              </button>
            ))}
          </div>
            ...
            <div>
              ...
              {aiData.summary && (
                <div style={{ background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-lg)", padding: "1.25rem", marginBottom: "1.25rem" }}>
                  <h3 style={{ margin: "0 0 8px", fontSize: 15, fontWeight: 500 }}><i className="ti ti-sparkles" style={{ marginRight: 6, color: "#534AB7" }} aria-hidden="true"></i>AI assessment</h3>
                    ...
                    <div style={{ marginTop: "1rem" }}>
                      ...
                      {aiData.quickWins.map((q, i) => (
                        <div key={i} style={{ display: "flex", gap: 8, marginBottom: 6 }}>
                          <i className="ti ti-bulb" style={{ fontSize: 14, color: "#EF9F27", flexShrink: 0, marginTop: 2 }} aria-hidden="true"></i>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "1.25rem", marginBottom: "1.25rem" }}>
                <div style={{ background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-lg)", padding: "1.25rem" }}>
                  ...
                  <div style={{ position: "relative", height: 160 }}>
                    <canvas id="ats-donut" role="img" aria-label={`ATS score donut chart: ${atsScore.total}%`}></canvas>
                    <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column" }}>
                      <span style={{ fontSize: 28, fontWeight: 500, color: scoreColor(atsScore.total) }}>{atsScore.total}</span>
                ...
                {jdSkills.length > 0 && (
                  <div style={{ background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-lg)", padding: "1.25rem" }}>
                    ...
                    <div style={{ position: "relative", height: 160 }}>
                      <canvas id="match-donut" role="img" aria-label={`Job description match: ${matchPct}%`}></canvas>
                      <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column" }}>
                        <span style={{ fontSize: 28, fontWeight: 500, color: scoreColor(matchPct) }}>{matchPct}%</span>
                        <span style={{ fontSize: 11, color: "var(--color-text-secondary)" }}>match</span>
                      </div>
                    </div>
                  </div>
                )}
                <div style={{ background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-lg)", padding: "1.25rem" }}>
                  ...
                  <div style={{ height: 140 }}>
                    <canvas id="skills-bar" role="img" aria-label="ATS score breakdown by category"></canvas>
                  </div>
                </div>
              </div>
                ...
                <div style={{ background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-lg)", padding: "1.25rem" }}>
                  ...
                  {[
                    ...
                    <div key={key} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                      <i className={found ? "ti ti-check" : "ti ti-x"} style={{ fontSize: 14, color: found ? "#1D9E75" : "#E24B4A", flexShrink: 0 }} aria-hidden="true"></i>
          ...
          {activeTab === "skills" && (
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.25rem" }}>
              <div style={{ background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-lg)", padding: "1.25rem" }}>
                <h3 style={{ margin: "0 0 12px", fontSize: 15, fontWeight: 500 }}><i className="ti ti-code" aria-hidden="true" style={{ marginRight: 6 }}></i>Technical skills ({skills.technical.length})</h3>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              ...
              <div style={{ background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-lg)", padding: "1.25rem" }}>
                <h3 style={{ margin: "0 0 12px", fontSize: 15, fontWeight: 500 }}><i className="ti ti-users" aria-hidden="true" style={{ marginRight: 6 }}></i>Soft skills ({skills.soft.length})</h3>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              ...
              {missingSkills.length > 0 && (
                <div style={{ gridColumn: "1 / -1", background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-lg)", padding: "1.25rem" }}>
                  <h3 style={{ margin: "0 0 8px", fontSize: 15, fontWeight: 500 }}><i className="ti ti-alert-triangle" aria-hidden="true" style={{ marginRight: 6, color: "#EF9F27" }}></i>Missing from JD ({missingSkills.length})</h3>
